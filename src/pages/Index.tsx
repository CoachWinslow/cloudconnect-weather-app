...
const Index = () => {
  ...
  const [mikesOpen, setMikesOpen] = useState(false);
  ...
  const mikesCities = useMemo(() => {
    if (!cities) return [];
    const byId = new Map(cities.map((c: any) => [c.id, c]));
    return MIKES_TOP_CITY_IDS.map((id) => byId.get(id)).filter(Boolean);
  }, [cities]);
    ...
    <div className="min-h-screen bg-background grid-bg relative">
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="space-y-4">
          {mikesCities.length > 0 && (
            <Collapsible open={mikesOpen} onOpenChange={setMikesOpen}>
              <CollapsibleTrigger className="w-full group">
                <div className="flex items-center gap-3 px-3 py-2 rounded-sm bg-card border border-primary/40 hover:border-primary/60 transition-colors cursor-pointer">
                  <ChevronDown className={`w-4 h-4 text-primary transition-transform duration-200 ${mikesOpen ? '' : '-rotate-90'}`} />
                  <span className="font-display text-sm font-semibold text-primary uppercase tracking-wider">
                    ★ Mike's Top Cities
                  </span>
                  <div className="flex-1 h-px bg-border" />
                  <span className="font-mono text-[10px] text-muted-foreground">
                    {mikesCities.length} {mikesCities.length === 1 ? 'station' : 'stations'}
                  </span>
                </div>
              </CollapsibleTrigger>
              <CollapsibleContent>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-3">
                  {mikesCities.map((city: any, i: number) => (
                    <CityCard
                      key={`mikes-${city.id}`}
                      city={city}
      ...
      <footer className="border-t border-border py-6 mt-10">
        <div className="container mx-auto px-4 flex flex-col items-center gap-4">
          <div className="flex items-center gap-5">
            <a href="https://github.com/CoachWinslow" target="_blank" rel="noopener noreferrer" className="group" aria-label="GitHub">
            ...
            <a href="https://x.com/CoachWinslow" target="_blank" rel="noopener noreferrer" className="group" aria-label="X (Twitter)">
              <svg viewBox="0 0 24 24" className="w-5 h-5 text-muted-foreground group-hover:text-foreground transition-colors" fill="currentColor">
            ...
            <a href="https://coachwinslow.slack.com" target="_blank" rel="noopener noreferrer" className="group" aria-label="Slack">
              <Slack className="w-5 h-5 text-muted-foreground group-hover:text-foreground transition-colors" />
            </a>
          </div>
